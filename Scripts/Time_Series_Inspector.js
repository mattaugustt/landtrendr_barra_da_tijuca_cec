...
Map.addLayer(barra, {}, 'Área de Estudo');

//imagem landtrendr:
var paleta = {bands: ['b4'], max: 1000, min: -10, opacity: 1, palette: ["cccccc","91ff04","fffa00","ff9c02","ff1a00"]};

var img = ee.Image("projects/ee-mataugusto1999/assets/lt_BarraTijuca_NDVI").clip(barra);
Map.addLayer(img, paleta, "LandTrendr Barra", false);


// pontos de validação gerados utilizando Pontos Aleatórios em Polígonos no QGIS
var pontos = ee.FeatureCollection("projects/ee-mattaugustt/assets/pontos_validacao_ltgee_barra")
Map.addLayer(pontos, {color: 'blue'}, "Pontos da Validação");
...
/*Author:He Yin
  ...
  Purpose: A tool for plotting NDVI, BSI (bare soil index) and Tasseled Cap transformation (Brightness, Greenness and Wetness) time series from Landsat SR and Sentinel 2 TOA time series; 
           visual interpretation of cloud-free Landsat SR imgery.
           ...
           The 2nd window shows the least clouded imagery for the period start1-end1
           The 3rd window shows the least clouded imagery for the period start2-end2
           The 4th window shows the least clouded imagery for the period start-end
  ...
  Parameters:
           ...
           Kernal size: whether NDVI value from a single pixel (30) or the mean value of the 3*3 (90) Landsat pixel should be plotted
...
/*Commit: Pedro de Castro
  Email: pbastosd@gmail.com
  Date: 08-March-2021
  Changes: 
          The 4th window was removed.
          Names Spring and Autumn were changed to Rain season and Dry season
*/

////Parameters definition 
//pick up an initial location
  var locat = ee.Geometry.Point(-43.39584, -22.9928); //point we have discussed

//Select the imgery during which period for plotting NDVI time series (first to my right)
  var start='1985-01-1';
  var end='2024-01-31';
//Select rain season imagery for the second window
  var start1='1985-01-01';
  var end1='1999-12-31'; 
//Select dry season imagery for the third window
  var start2='2010-01-01';
  var end2='2020-12-31';
...
//Calculate NDVI
  ...
  var ndbi =image.normalizedDifference(['swir2','nir']).rename('L8_SR_NDBI');
  return image.addBands(ndbi); //podemos utilizar swir 1 ou 2.
};
...
//Apply the cloud mask on L8
  var collectionL8_ndvi = ee.ImageCollection('LANDSAT/LC08/C02/T1_L2')
    ...
    .map(bsiL8)
    .select('L8_SR_NDVI');
  var collectionL8_ndbi = ee.ImageCollection('LANDSAT/LC08/C02/T1_L2')
    .filterDate(start, end)
    .filter(ee.Filter.calendarRange(startmonth,endmonth,'month'))
    .map(cloudMaskL8)
    .map(addNDBI)
  ...
  var collectionL8_green = ee.ImageCollection('LANDSAT/LC08/C02/T1_L2')
    .filterDate(start, end)
    .filter(ee.Filter.calendarRange(startmonth,endmonth,'month'))
    .map(cloudMaskL8)
    .map(greenessL8)
...
// Remove edge pixels that don't occur in all bands
 //var mask2 = image.mask().reduce(ee.Reducer.min());
  return image.select(['SR_B1', 'SR_B2', 'SR_B3', 'SR_B4', 'SR_B5', 'SR_B7'], // change band names
...
var addNDBI= function(image){
  var ndbi =image.normalizedDifference(['swir1','nir']).rename('L4_7_SR_NDBI');
...
//Calculate NDVI, NDBI, bsi, TC (brightness, greenness and wetness) from L7 and L5
  ...
  var collectionL4_ndvi = ee.ImageCollection('LANDSAT/LT04/C02/T1_L2')
   ...
   .filter(ee.Filter.calendarRange(startmonth,endmonth,'month'))
    ...
    .map(bsiL7)
    .select('L4_7_SR_NDVI');    
   var collectionL7_ndbi = ee.ImageCollection('LANDSAT/LE07/C02/T1_L2')
    .filterDate(start, end)
    .filter(ee.Filter.calendarRange(startmonth,endmonth,'month'))
    .map(cloudMaskL457)
    .map(addNDBI)
    .map(bsiL7)
    .select('L4_7_SR_NDBI');  
  var collectionL5_ndbi = ee.ImageCollection('LANDSAT/LT05/C02/T1_L2')
    .filterDate(start, end)
   .filter(ee.Filter.calendarRange(startmonth,endmonth,'month'))
    .map(cloudMaskL457)
    .map(addNDBI)
    .map(bsiL7)
    .select('L4_7_SR_NDBI');  
  var collectionL4_ndbi = ee.ImageCollection('LANDSAT/LT04/C02/T1_L2')
    .filterDate(start, end)
   .filter(ee.Filter.calendarRange(startmonth,endmonth,'month'))
    .map(cloudMaskL457)
    .map(addNDBI)
  ...
  var collectionL4_bright = ee.ImageCollection('LANDSAT/LT04/C02/T1_L2')
   ...
   .filter(ee.Filter.calendarRange(startmonth,endmonth,'month'))
    ...
    .map(brightL7)
    .select('L4_7_SR_brightness');
  var collectionL7_green = ee.ImageCollection('LANDSAT/LE07/C02/T1_L2')
    .filterDate(start, end)
    .filter(ee.Filter.calendarRange(startmonth,endmonth,'month'))
    .map(cloudMaskL457)
    .map(greenL7)
    .select('L4_7_SR_greeness');    
  var collectionL5_green = ee.ImageCollection('LANDSAT/LT05/C02/T1_L2')
    .filterDate(start, end)
   .filter(ee.Filter.calendarRange(startmonth,endmonth,'month'))
    .map(cloudMaskL457)
    .map(greenL7)
    .select('L4_7_SR_greeness');   
  var collectionL4_green = ee.ImageCollection('LANDSAT/LT04/C02/T1_L2')
    .filterDate(start, end)
   .filter(ee.Filter.calendarRange(startmonth,endmonth,'month'))
    .map(cloudMaskL457)
    .map(greenL7)
...
// Caculate Sentinel 2 TOA NDVI, basi and TC
  var addNDBI= function(image){
  var ndbi =image.normalizedDifference(['B11','B8']).rename('S2_TOA_NDBI');
  return image.addBands(ndbi); //pode se usar as bandas B11 e B12
};
...
// Load Sentinel-2 TOA reflectance data.
  var collectionS_ndbi = ee.ImageCollection('COPERNICUS/S2_HARMONIZED')
    .filterDate(start, end)
    .filter(ee.Filter.calendarRange(startmonth,endmonth,'month'))
    ...
    .map(bsiS2)
    .select('S2_TOA_NDBI');
  var collectionS_ndvi = ee.ImageCollection('COPERNICUS/S2_HARMONIZED')
    .filterDate(start, end)
    .filter(ee.Filter.calendarRange(startmonth,endmonth,'month'))
    .filter(ee.Filter.lt('CLOUDY_PIXEL_PERCENTAGE', 20))
    .map(maskS2clouds)
    ...
    .map(bsiS2)
    .select('S2_TOA_NDVI');
  var collectionS_bsi = ee.ImageCollection('COPERNICUS/S2_HARMONIZED')
    .filterDate(start, end)
    .filter(ee.Filter.calendarRange(startmonth,endmonth,'month'))
    .filter(ee.Filter.lt('CLOUDY_PIXEL_PERCENTAGE', 20))
    .map(maskS2clouds)
    ...
    .map(bsiS2)
    .select('S2_TOA_bsi');      
  var collectionS_wet = ee.ImageCollection('COPERNICUS/S2_HARMONIZED')
    .filterDate(start, end)
    .filter(ee.Filter.calendarRange(startmonth,endmonth,'month'))
    .filter(ee.Filter.lt('CLOUDY_PIXEL_PERCENTAGE', 20))
    .map(maskS2clouds)
  ...
  var collectionS_bright = ee.ImageCollection('COPERNICUS/S2_HARMONIZED')
    .filterDate(start, end)
    .filter(ee.Filter.calendarRange(startmonth,endmonth,'month'))
    .filter(ee.Filter.lt('CLOUDY_PIXEL_PERCENTAGE', 20))
    .map(maskS2clouds)
  ...
  var collectionS_green = ee.ImageCollection('COPERNICUS/S2_HARMONIZED')
    .filterDate(start, end)
    .filter(ee.Filter.calendarRange(startmonth,endmonth,'month'))
    .filter(ee.Filter.lt('CLOUDY_PIXEL_PERCENTAGE', 20))
    .map(maskS2clouds)
...
//Merge all the collections
  var collection_ndbi = collectionL8_ndvi.merge(collectionL5_ndbi)
                       .merge(collectionL7_ndbi).merge(collectionS_ndbi);                       
...
//Merge all the collections
 ...
 //Get rain season imagery (rain) and dry season imgery (dry) collection
  var collection_rain=collection.filterDate(start1,end1).sort('CLOUD_COVER', true);
  var collection_dry=collection.filterDate(start2,end2).sort('CLOUD_COVER', true);
    
//Get the imagery list
  var imagery_list =collection.toList(collection.size());
  print('Imagery List',imagery_list); 
  var imagery_list_rain =collection_rain.toList(collection_rain.size());
  print('Rain season imagery List',imagery_list_rain);
  var imagery_list_dry =collection_dry.toList(collection_dry.size());
  print('Dry season imagery List',imagery_list_dry);

//get the *the clouded imagery from the list, 'cloudness' was defined at the beginning
  var image0_rain =ee.Image(imagery_list_rain.get(cloudness));
  var name0_rain_1 =ee.String(image0_rain.id());
  var strlength_rain=name0_rain_1.length()
  var name0_rain = name0_rain_1.slice(6,strlength_rain);
  var id0_rain=name0_rain.getInfo(1);
  
  var image0_dry =ee.Image(imagery_list_dry.get(cloudness));
  var name0_dry_1 =ee.String(image0_dry.id());
  var strlength_dry=name0_dry_1.length()
  var name0_dry = name0_dry_1.slice(6,strlength_dry);
  var id0_dry=name0_dry.getInfo();
    
//Take the least clouded 5 imagery and sort them based on the observation time    
  var image5_rain = collection_rain.limit(2).sort('SENSING_TIME',true);
  var image5_dry = collection_dry.limit(2).sort('SENSING_TIME',true);

//Set the band combination and color scheme
  var vis2={bands: ["SWIR2", "NIR", "red"], min:0, max: 5000};
  var imageclear0_rain = ee.Image(image0_rain).visualize(vis2);
  var imageclear0_dry = ee.Image(image0_dry).visualize(vis2);
...
// Create an intro panel with labels.
  var intro = ui.Panel([
  ui.Label({
    value: 'NDVI/NDBI/BSI/TC Time Series Inspector',
    style: {fontSize: '20px', fontWeight: 'bold'}
  }),
  ui.Label('Click a location to see NDVI/NDBI/BSI/TC time series from Landsat SR and Sentinel-2 TOA')
]);
  mapPanel.add(intro);
...
//criando a paleta
var Vis1 = {bands: ['red', 'green', 'blue'], gamma:1, max: 22198, min: 7598, opacity: 1};
var Vis2 = {bands: ['red', 'green', 'blue'], gamma:1, max: 21405, min: 7442, opacity: 1};


// criando a coleção para cada periodo:
var collection1=collection.filterDate('1985-01-01','1990-12-31').sort('CLOUD_COVER', true);
var collection2=collection.filterDate('2010-01-01','2015-12-31').sort('CLOUD_COVER', true);
//var leastCloudyImage1 = collection1.sort('CLOUDY_PIXEL_PERCENTAGE').first();
//var leastCloudyImage2 = collection2.sort('CLOUDY_PIXEL_PERCENTAGE').first();

//adicionando as duas outras imagens
map1.addLayer(collection1.first(), Vis1, 'Período 2');
map2.addLayer(collection2.first(), Vis2, 'Período 3');
...
// Make a chart from the ndbi time series.
  var ndbiChart = ui.Chart.image.series(collection_ndbi, point, ee.Reducer.mean(), kernalsize);
// Customize the chart.
  ndbiChart.setOptions({
    title: 'NDBI: time series',
    vAxis: {title: 'NDBI',ticks:[-0.1,0,0.2,0.4,0.6,0.8,1]},
...
// Add the chart at a fixed position, so that new charts overwrite older ones.
    ...
    mapPanel.widgets().set(3, ndbiChart);
    mapPanel.widgets().set(4, bsiChart);
    mapPanel.widgets().set(5, wetChart);
    mapPanel.widgets().set(6, brightChart);
    mapPanel.widgets().set(7, greenChart);
    
    map1.setCenter(point.coordinates().get(0).getInfo(),point.coordinates().get(1).getInfo(), 14);
    //map1.addLayer(collection_rain, vis2, 'collection_rain');
    map1.addLayer(point, {color:'FF0000'}, 'point');
    map1.add(ui.Label(id0_rain, {position:'bottom-center'}));
  
    map2.setCenter(point.coordinates().get(0).getInfo(),point.coordinates().get(1).getInfo(), 14);
    //map2.addLayer(image0_dry, vis2, 'image0_dry');
    map2.addLayer(point, {color:'FF0000'}, 'point');
    map2.add(ui.Label(id0_dry, {position:'bottom-center'}));
};
...
var addButton = ui.Button('Adicionar Ponto', function() {
  var idValue = idInput.getValue();
  if (idValue) {
    drawingTools.setShape('point'); // Definir a forma como ponto
    drawingTools.start(); // Iniciar o desenho
  } else {
    print('Por favor, insira um ID.');
  }
});
panel.add(addButton);


var sampV_teste1 = nao_mudou.merge(mudou); //adicionar os pontos aleatorios
Map.addLayer(sampV_teste1, {color: 'blue'}, 'Pontos validados', false);
print(sampV_teste1)


//exportando os pontos de validação para os assets
Export.table.toAsset({
  collection: sampV_teste1,
  description:'ExportValidationSamples',
  assetId: 'others_filtered',//asset's name
});
